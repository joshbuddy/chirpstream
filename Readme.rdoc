...
== Usage
  ...
  chirp = Chirpstream.basic('joshbuddy', 'xxxxxxxx')
...
== Oauth Usage
  ...
  chirp = Chirpstream.oauth(consumer_token, consumer_secret, access_token, access_secret)
  chirp.reconnect{ puts "reconnecting..." }
  chirp.tweet {|t| puts "#{t.text} (from #{t.user.name.foreground(:red)} (#{('@' + t.user.screen_name).foreground(:green)}))" }
  chirp.follow {|t| puts "#{t.source.screen_name.foreground(:green)} following #{t.target.screen_name.foreground(:green)}" }
  chirp.favorite {|t| puts "#{t.source.screen_name.foreground(:green)} <3 -> #{t.target_object.text}" }
...
class User
	attr_accessor :twitter_access_token, :twitter_secret_token
	def initialize(access_token,secret_token)
		@twitter_access_token = access_token
		@twitter_secret_token = secret_token
	end
end

chirp = Chirpstream.new(nil,nil,false)
...
user1 = User.new('XXX', "XXX")
user2 = User.new('XXX', 'XXX')

chirp.multi_connect(:users => [user1, user2])
